"""Routage comptable membre/client (PS3) — l'épargne d'un membre va sur 3111, celle d'un
simple client sur 3112, ANCRÉ à l'ouverture du compte.

  - membre ouvre -> ancré 3111, dépôt -> écriture C 3111 ;
  - client ouvre -> ancré 3112, dépôt -> écriture C 3112 ;
  - client devient membre -> ses comptes existants RESTENT sur 3112 (aucun re-routage, aucun
    transfert automatique — option B, à valider expert) ; un NOUVEAU compte part sur 3111 ;
  - repli : produit sans rattachement client -> tout sur 3111 (comportement historique intact) ;
  - rapprochement PAR GROUPE : Σ soldes ancrés 3111 == 3111 ET Σ soldes ancrés 3112 == 3112 ;
  - les intérêts suivent l'ancrage (un compte client est crédité sur 3112).
...
def _cadre(db: Session, suffixe: str, *, avec_client: bool = True, taux_bp: int = 0):
    """Agence + produit rattaché 3111 (membre) et, si demandé, 3112 (client)."""
    agence = Agency(code=f"AGR-{suffixe}", name="Agence", compte_caisse_id=_cid(db, "5721"))
    produit = Product(
        code=f"PR{suffixe}", name="Épargne", type="a_vue",
        compte_epargne_id=_cid(db, "3111"),
        compte_epargne_client_id=_cid(db, "3112") if avec_client else None,
        compte_charge_interet_id=_cid(db, "603"),
...
def test_membre_ouvre_ancre_3111_et_depose_sur_3111(db: Session) -> None:
    agence, produit = _cadre(db, "M1")
    tier_id = _tier(db, agence.id, "M1", membre=True)
    compte = _ouvrir(db, produit, agence, tier_id)

    assert _ancre(db, compte.id) == "3111"
    deposer(db, _caissier(db, agence.id), compte.id, 10000)
    assert ("3111", "C", 10000) in _lignes_epargne(db, compte.id)


def test_client_ouvre_ancre_3112_et_depose_sur_3112(db: Session) -> None:
    agence, produit = _cadre(db, "C1")
    tier_id = _tier(db, agence.id, "C1", membre=False)
    compte = _ouvrir(db, produit, agence, tier_id)

    assert _ancre(db, compte.id) == "3112"
    deposer(db, _caissier(db, agence.id), compte.id, 8000)
    lignes = _lignes_epargne(db, compte.id)
    assert ("3112", "C", 8000) in lignes
    assert not any(n == "3111" for n, _s, _m in lignes)  # rien ne fuit vers le compte membre


def test_client_devenu_membre_ses_comptes_restent_sur_3112(db: Session) -> None:
    agence, produit = _cadre(db, "D1")
    tier_id = _tier(db, agence.id, "D1", membre=False)
    caissier = _caissier(db, agence.id)
    ancien = _ouvrir(db, produit, agence, tier_id)  # ouvert en CLIENT -> 3112
    ...
    # Son compte EXISTANT reste ancré 3112 : les nouveaux mouvements y vont toujours.
    deposer(db, caissier, ancien.id, 3000)
    assert _ancre(db, ancien.id) == "3112"
    lignes = _lignes_epargne(db, ancien.id)
    assert ("3112", "C", 5000) in lignes and ("3112", "C", 3000) in lignes
    assert not any(n == "3111" for n, _s, _m in lignes)  # aucun re-routage, aucun transfert

    # Un NOUVEAU compte, ouvert membre, part sur 3111.
    nouveau = _ouvrir(db, produit, agence, tier_id)
    assert _ancre(db, nouveau.id) == "3111"
...
def test_repli_produit_sans_rattachement_client(db: Session) -> None:
    ...
    # comportement historique, intact tant que 3112 n'est pas configuré.
    ...
    assert ("3111", "C", 2000) in _lignes_epargne(db, compte.id)


def test_rapprochement_concorde_par_groupe(db: Session) -> None:
    agence, produit = _cadre(db, "G1")
    caissier = _caissier(db, agence.id)
    base_m = rapprocher(db, _cid(db, "3111"))  # la base de dev vit : deltas, pas d'absolus
    base_c = rapprocher(db, _cid(db, "3112"))
    ...
    apres_m = rapprocher(db, _cid(db, "3111"))
    apres_c = rapprocher(db, _cid(db, "3112"))
...
def test_les_interets_d_un_client_suivent_l_ancrage_3112(db: Session) -> None:
    # 10 % l'an base 365 : 100 000 déposés toute l'année -> 10 000 d'intérêts, crédités sur 3112.
    ...
    lignes = _lignes_epargne(db, compte.id)
    assert ("3112", "C", 10000) in lignes  # la dette envers le CLIENT monte sur SON collectif
    assert ("603", "D", 10000) in lignes
