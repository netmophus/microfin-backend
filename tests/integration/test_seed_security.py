...
def test_les_donnees_declarees_sont_coherentes() -> None:
    ...
    assert len(ROLES) == 11
    # 18 Sécurité + 14 Tiers (10 + 4 parts) + 7 Compta + 9 Épargne + 5 Crédit (2 produit + 3 demande)
    assert len(PERMISSIONS) == 53
...
def test_une_permission_hors_matrice_est_revoquee(session: Session) -> None:
    ...
    # Le droit hors matrice disparaît ; le caissier reconverge vers sa matrice.
    assert _codes_accordes(session, "CAISSIER") == {
        "tiers.read.basic",
